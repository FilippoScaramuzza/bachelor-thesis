...
class ExperimentConfiguration:
    ...
    def __init__(self, expDir, param_to_change = None, new_value = None, x = None):
        ...
        self.REDUCTION_FACTOR_2 = 4/3 # FOG-i -> FOG-i+1 nodes reduction factor (3/2 means multuplying by 2/3)
        self.HUB_GENERATION_PROBABILITY = 0.1
        self.ALL_TO_CLOUD = False
        self.MIN_CONN_TO_UPPER_LEVELS = 20
        self.MAX_CONN_TO_UPPER_LEVELS = 30

        self.PRIVACY_ASSIGNEMENT = "random.randrange(0,100) <= 20"
        ...
        if param_to_change != None:
            exec(f'self.{param_to_change} = {new_value}')
            if param_to_change == 'MIN_CONN_TO_UPPER_LEVELS':
                self.MAX_CONN_TO_UPPER_LEVELS = new_value + 10
            if param_to_change == 'FUNC_NODE_RAM_FOG0':
                self.FUNC_NODE_RAM_FOG1 = round(self.FUNC_NODE_RAM_FOG0 * 1.13)
                self.FUNC_NODE_RAM_FOG2 = round(self.FUNC_NODE_RAM_FOG0 * 1.13)
                self.FUNC_NODE_RAM_FOG3 = round(self.FUNC_NODE_RAM_FOG0 * 1.13)
                self.FUNC_NODE_RAM_FOG0 = str(self.FUNC_NODE_RAM_FOG0)
        ...
        file = open(self.expDirectory + "configurationUsed.json","w")
    ...
    def userGeneration(self):
        ...
        probs_of_reqs = np.load(str(path.absolute()) + '/probs.npy')
        ...
        for i in range(0,self.NUMBER_OF_APPS):
            ...
            for j in self.gateways_devices:
                ...
                if probs_of_reqs[j]<prob_of_requested:
                    one_user={}
                    one_user['app']=str(i)
