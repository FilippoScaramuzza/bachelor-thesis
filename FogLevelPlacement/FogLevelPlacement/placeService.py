...
class ServicePlacement:
    ...
    def solve(self):
        ...
        allAlloc = {}
        self.servicePerLevel = [0 for i in range(0, 7)]
            ...
            for idDevice in range(len(self.ec.G.nodes)):
                if self.service2DevicePlacementMatrix[idServ][idDevice]==1:
                    ...
                    # Counting Nodes
                        ...
                        self.servicePerLevel[self.ec.G.nodes[idDevice]["level[z]"]] +=1 
        ...
        file = open(self.resultFolder+"/placementResult.json", "w")
        file.write(json.dumps(placementResults))
        file.close()

        file = open(self.resultFolder+"/placementResults.json")
        history = json.loads(file.read())
        history.append(placementResults)
        file = open(self.resultFolder+"/placementResults.json", "w")
    ...
    def devicesFirstFitDescendingOrder(self, nodes, source_node, appId):
        ...
        for devId in nodes:
            ...
            # Only for fog nodes
            if self.ec.G.nodes[devId]["level[z]"] in [2,3,4,5,6]:
...
def run(param_to_change=None, new_value=None, x=None):
    ...
    abs_path_plot = str(path_plots.absolute()) + "/"
    ec = ExperimentConfiguration(abs_path, param_to_change, new_value, x)
    ...
    plot_.plotServicePlacementPerLevel()
